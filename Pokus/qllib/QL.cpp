...
QL::QL(Stav* stav, int pocFeatur, int pocAkcii,QInit* qin)
{
	...
	stavy->insert(make_pair(vec, qStav));

    cout << vec->dajStavConst()->capacity() << endl;
    cout << stav->dajStavConst()->capacity() << endl;

	pocetAkcii = pocAkcii;
    pocetFeatur = pocFeatur;
    pocetUceni = 0;

    /*for (auto value : *stavy)
    {
        cout << value.first->dajStavConst()->capacity() << endl;
    }*/
